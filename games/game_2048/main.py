...
def draw_scores(score, best_score):
	score_text = FONT.render(f"Score: {score}", True, (255, 255, 255))
	best_text = FONT.render(f"Best: {best_score}", True, (255, 255, 255))
	screen.blit(score_text, (10, HEIGHT - 40))
	screen.blit(best_text, (WIDTH - best_text.get_width() - 10, HEIGHT - 40))


# grid dispay
def draw_grid(grid, score, best_score):
...
def main(scores: Scores):
	score = 0
	g = scores.get()
	best_score = max(g) if len(g) != 0 else 0
	...
	draw_grid(grid, score, best_score)
...
def run(screen_in: pygame.Surface, scores: Scores):
	global screen
